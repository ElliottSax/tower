...
function SecurityManager.BanPlayer(player: Player, reason: string, durationOverride: number?)
	...
	if not duration then
		if history.OffenseCount >= CONFIG.PermanentBanAfterOffenses then
			...
			-- SECURITY: Cap exponent to prevent overflow (max 10 offenses before max duration)
			local safeExponent = math.min(history.OffenseCount - 1, 10)
			duration = CONFIG.BanDurationBase * math.pow(CONFIG.BanDurationMultiplier, safeExponent)
			-- Clamp to max duration and ensure it's a valid number
			if duration ~= duration or duration == math.huge then
...
function SecurityManager.IsBanned(player: Player): boolean
	...
	-- Use the comprehensive ban check that includes DataStore lookup
	return SecurityManager.CheckPlayerBan(userId)
end
...
function SecurityManager.StartMonitoring()
	...
	Players.PlayerRemoving:Connect(function(player)
		...
		-- Rate limits use composite keys "userId:action", so clean up by prefix
		local prefix = tostring(player.UserId) .. ":"
		local prefixLen = #prefix
		for key, _ in pairs(SecurityManager.RateLimits) do
			if type(key) == "string" and string.sub(key, 1, prefixLen) == prefix then
				SecurityManager.RateLimits[key] = nil
			end
		end
	end)
...
function SecurityManager.LoadBans()
	-- Note: Regular DataStores cannot list all keys.
	-- Bans are loaded on-demand when players join via CheckPlayerBan().
	-- This function initializes the ban system and can be extended to load
	-- a known list of banned users from a configuration if needed.
	print("[SecurityManager] Ban system initialized (on-demand loading)")
end

-- Check if a specific player is banned (called on join)
function SecurityManager.CheckPlayerBan(userId: number): boolean
	-- Check in-memory cache first
	if SecurityManager.Bans[userId] then
	...
	-- Check DataStore for persistent ban
	local banDataStore = DataStoreService:GetDataStore("SecurityBans")
	...
	if success and banData then
		-- Check if ban expired
		if banData.Duration > 0 then
			local elapsed = tick() - banData.Timestamp
		...
		-- Cache the ban in memory
		SecurityManager.Bans[userId] = banData

		-- Also load ban history if available
		if banData.OffenseCount then
			SecurityManager.BanHistory[userId] = {
				OffenseCount = banData.OffenseCount,
				LastBan = banData.Timestamp
			}
		end
