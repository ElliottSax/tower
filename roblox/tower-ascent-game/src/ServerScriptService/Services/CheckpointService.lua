...
function CheckpointService.OnCharacterAdded(player: Player, character: Model)
	...
	-- Listen for death
	-- Disconnect old character connections first to prevent accumulation across respawns
	if CheckpointService.CharacterConnections[player] then
		for _, connection in ipairs(CheckpointService.CharacterConnections[player]) do
			connection:Disconnect()
		end
...
function CheckpointService.OnCheckpointTouched(checkpoint: BasePart, hit: BasePart)
	...
	-- SECURITY: Validate section number is within valid bounds
	if type(section) ~= "number" or section < 1 or section > GameConfig.Tower.SectionsPerTower then
		warn(string.format(
			"[CheckpointService] Invalid section number: %s (expected 1-%d)",
			tostring(section),
...
function CheckpointService.OnFinishLineTouched(finishLine: BasePart, hit: BasePart)
	...
	-- Visual celebration
	task.spawn(function()
		if not finishLine or not finishLine.Parent then return end
...
function CheckpointService.StartDebounceCleanupLoop()
	task.spawn(function()
		while true do
			task.wait(30) -- Every 30 seconds (more frequent to prevent memory buildup)
