...
function CoinService.AddCoins(player: Player, amount: number, source: string?): boolean
	...
	if success and VIPService and VIPService.GetCoinMultiplier then
		...
		-- VIP only provides 2x coins, anything higher indicates compromise
		if type(multiplier) ~= "number" or multiplier ~= multiplier then
			warn("[CoinService] Invalid multiplier from VIPService:", multiplier)
			multiplier = 1
		end

		-- Clamp multiplier to valid range (1x base, 2x max for VIP)
		-- Higher values would indicate VIPService compromise or exploit
		multiplier = math.clamp(multiplier, 1, 2)
	end
	...
	local addSuccess = DataService.AddCoins(player, amount)
