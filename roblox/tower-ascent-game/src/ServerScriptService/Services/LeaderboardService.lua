...
function LeaderboardService.FetchLeaderboard(leaderboardType: string)
	...
	local success, pages = pcall(function()
		return OrderedDataStores[leaderboardType]:GetSortedAsync(false, 100)
	end)
	...
	local fetchSuccess, err = pcall(function()
...
function LeaderboardService.OnPlayerAdded(player: Player)
	-- Guard: player may have left during the task.wait(2) delay before this is called
	...
	if profile and profile.Data then
		...
		local highestStage = stats.HighestStage or 0
		local towersCompleted = stats.TowersCompleted or 0
		local totalCoins = stats.TotalCoinsEarned or data.Coins or 0

		-- Update all leaderboard stats
		LeaderboardService.UpdatePlayerStat(player.UserId, "HighestStage", highestStage)
		LeaderboardService.UpdatePlayerStat(player.UserId, "TowersCompleted", towersCompleted)
		LeaderboardService.UpdatePlayerStat(player.UserId, "TotalCoins", totalCoins)
