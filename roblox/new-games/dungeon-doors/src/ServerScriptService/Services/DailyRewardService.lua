--[[
	DailyRewardService.lua - Dungeon Doors
	7-day streak daily rewards with souls and items
]]
...
local REWARDS = {
	{ Day = 1, Type = "Souls", Amount = 50 },
	{ Day = 2, Type = "Souls", Amount = 100 },
	{ Day = 3, Type = "Souls", Amount = 200 },
	{ Day = 4, Type = "Souls", Amount = 300 },
	{ Day = 5, Type = "Souls", Amount = 500 },
	{ Day = 6, Type = "Souls", Amount = 750 },
	{ Day = 7, Type = "Souls", Amount = 1500 },
}
...
function DailyRewardService.Init()
	...
	re:WaitForChild("ClaimDaily").OnServerEvent:Connect(function(player)
		...
		if reward and reward.Type == "Souls" then
			data.Souls = (data.Souls or 0) + reward.Amount
		end

		local reEvents = RS:FindFirstChild("RemoteEvents")
		if reEvents then
			local du = reEvents:FindFirstChild("DailyUpdate")
			if du then du:FireClient(player, { Day = data.DailyStreak, Reward = reward, Souls = data.Souls }) end
		end
	end)
