 ...
 * Best for: Quick transformations, API requests, edge computing
 */
...
class CloudflareProvider extends BaseProvider {
  ...
  async request(endpoint, method = 'GET', body = null) {
    ...
    if (!response.ok) {
      ...
      throw this.buildError(`Cloudflare error: ${errorMsg}`, response.status);
    }
  ...
  estimateCost(task) {
    ...
    // Plus $0.02 per million GB-seconds of duration
    const baseCost = 0.0000005; // $0.50 / 1M

    // Add duration cost based on task type
    const durationCosts = {
      'hash': 0.0000001,
      'base64': 0.0000001,
